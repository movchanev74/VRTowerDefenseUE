...
void UGameplayAbilityWeaponFire::ApplyWeaponFireEffect(FGameplayAbilityTargetDataHandle TargetData, TSubclassOf<UGameplayEffect> EffectClass)
{
	// Проверка, что передан корректный класс эффекта
	if (EffectClass)
	{
		FWeaponFireGameplayEffectContext* WeaponFireContext = new FWeaponFireGameplayEffectContext();
		FGameplayEffectContextHandle ContextHandle = FGameplayEffectContextHandle(WeaponFireContext);

		// Извлекаем информацию о попаданиях
		TArray<FVector> HitLocations;
		ExtractHitLocationsFromTargetData(TargetData, HitLocations);

		// Добавляем все позиции попаданий в кастомный контекст
		for (int32 i = 0; i < HitLocations.Num(); i++)
		{
			WeaponFireContext->AddHitLocation(HitLocations[i]);
		}

		// Создаем спецификацию эффекта и передаем кастомный контекст
		FGameplayEffectSpecHandle SpecHandle = MakeOutgoingGameplayEffectSpec(EffectClass, GetAbilityLevel());
		SpecHandle.Data->SetContext(ContextHandle); // Устанавливаем кастомный контекст в спецификацию




		// Создаем FGameplayEffectSpec для выбранного эффекта
		//FGameplayEffectSpecHandle SpecHandle = MakeOutgoingGameplayEffectSpec(EffectClass, GetAbilityLevel());

		//// Получаем контекст из спецификации эффекта
		//FGameplayEffectContextHandle ContextHandle = SpecHandle.Data->GetEffectContext();

		//FWeaponFireGameplayEffectContext WeaponFireContext = 
		//{
		//	TArray<FVector> HitLocations;
		//	ExtractHitLocationsFromTargetData(TargetData, HitLocations);
		...
		//	}
		//}
		//SpecHandle.Data->SetContext()



		// Привязываем туда нужные данные (например, массив точек попаданий)
		//if (FWeaponFireGameplayEffectContext* WeaponFireContext = static_cast<FWeaponFireGameplayEffectContext*>(ContextHandle.Get()))
		//{
		//	TArray<FVector> HitLocations;
		//	ExtractHitLocationsFromTargetData(TargetData, HitLocations);
		//	for (int i = 0; i < HitLocations.Num(); i++)
...
//FWeaponFireGameplayEffectContext UGameplayAbilityWeaponFire::GetWeaponFireEffectContextFromHandle(const FGameplayEffectContextHandle& ContextHandle) const
//UFUNCTION(BlueprintCallable, Category = "Ability|Weapon")
//FWeaponFireGameplayEffectContext GetWeaponFireEffectContextFromHandle(const FGameplayEffectContextHandle& ContextHandle)
//{
//	const FGameplayEffectContext* BaseContext = ContextHandle.Get();
//	const FWeaponFireGameplayEffectContext* CustomContext = static_cast<const FWeaponFireGameplayEffectContext*>(BaseContext);
//
//	if (CustomContext)
//	{
//		return *CustomContext;  // Возвращаем копию структуры
//	}
//
//	return FWeaponFireGameplayEffectContext();  // Возвращаем пустую структуру, если каст не удался
//}

FWeaponFireGameplayEffectContext UGameplayAbilityWeaponFire::GetWeaponFireEffectContextFromHandle(const FGameplayEffectContextHandle& ContextHandle)
{
	const FGameplayEffectContext* BaseContext = ContextHandle.Get();
	if (BaseContext == nullptr)
	{
		return FWeaponFireGameplayEffectContext(); // Возвращаем пустой контекст, если BaseContext нулевой
	}
	...
	//const FWeaponFireGameplayEffectContext* CustomContext = (FWeaponFireGameplayEffectContext) BaseContext;
	//const FWeaponFireGameplayEffectContext* CustomContext = dynamic_cast<const FWeaponFireGameplayEffectContext*>(BaseContext);
	if (CustomContext)
	{
		return *CustomContext;  // Возвращаем копию структуры
	}
...
void UGameplayAbilityWeaponFire::ExtractHitLocationsFromTargetData(const FGameplayAbilityTargetDataHandle TargetData, TArray<FVector>& OutHitLocations)
{
	// Проходим по всем TargetData
	for (int32 i = 0; i < TargetData.Num(); ++i)
	{
		const FGameplayAbilityTargetData* TargetDataInstance = TargetData.Get(i);

		// Проверяем, является ли TargetDataInstance типом FLyraGameplayAbilityTargetData_SingleTargetHit
		const FLyraGameplayAbilityTargetData_SingleTargetHit* HitData = static_cast<const FLyraGameplayAbilityTargetData_SingleTargetHit*>(TargetDataInstance);
		if (HitData)
		{
			// Если это так, извлекаем HitResult и берем точку попадания
			const FHitResult& HitResult = HitData->HitResult;
			OutHitLocations.Add(HitResult.ImpactPoint);
		}
	}
}
